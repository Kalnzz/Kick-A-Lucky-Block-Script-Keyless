...
if not success or not Rayfield then
    warn("[AutoPurchase] Gagal load Rayfield!")
    return
end
...
Tab:CreateSection("All Tycoon 1 - 50")

local running = false

Tab:CreateButton({
    Name = "Start Auto Purchase",
    Callback = function()
        if running then
            Rayfield:Notify({
                Title = "Info",
                Content = "Masih berjalan, tunggu selesai!",
                Duration = 3,
            })
            return
        end
        ...
        Rayfield:Notify({
            Title = "Selesai!",
            Content = "Purchase selesai di " .. count .. " tycoon.",
            Duration = 5,
        })
    end,
})
